...
@Component({
  selector: 'app-book-trash-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
    ...
    <div class="trash-dialog">
      ...
      <mat-dialog-content>
        ...
        <div *ngIf="!loading && trashedChapters.length === 0" class="empty-state">
          <mat-icon>delete_outline</mat-icon>
          <p>No items in trash for this book</p>
        </div>
...
export class BookTrashDialogComponent implements OnInit {
  trashedChapters: Chapter[] = [];
  loading = true;

  constructor(
    public dialogRef: MatDialogRef<BookTrashDialogComponent>,
  ...
  loadTrashedChapters() {
    this.loading = true;
    this.chapterService.getBookTrashChapters(this.data.bookId).subscribe({
      next: (response) => {
